      ...
      drawPositions(ctx, newPositions, canvas) {
        ...
        for (const index in this.groupedPositions) {
          ...
          for (let i = 1; i < playerPositions.length; i++) {
            ...
            const y = mapCentreY + ((-position.posZ + this.offsetY) * this.scaleY);

            const prevX = mapCentreX + ((prevPosition.posX + this.offsetX) * this.scaleX);
            const prevY = mapCentreY + ((-prevPosition.posZ + this.offsetY) * this.scaleY);
          ...
          // Draw all the arrows and dots in reverse order
          for (let i = playerPositions.length - 1; i >= 0; i--) {
            const position = playerPositions[i];
  
            // Skip if the current position is the same as the next one
            if (i > 0) {
            ...
            // Calculate the dot and arrow size based on the position index
            const dotSize = Math.max(2, 5 - i * 0.2);
            const arrowLength = Math.max(20, 30 - i * 2);
            const arrowWidth = Math.max(6, 10 - i * 0.5);
